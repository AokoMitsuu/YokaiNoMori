...
public class Board : MonoBehaviour, IGameManager
{
    ...
    private IEnumerator EndScreen(ECampType pWiner)
    {
        ...
        {
            yield return ShowLabel(P1_Color, "Joueur 1 a gagn�", m_VictoryScreenDelay);
        }
        else if (pWiner == ECampType.PLAYER_TWO)
        {
            yield return ShowLabel(P2_Color, "Joueur 2 a gagn�", m_VictoryScreenDelay);
        }
        else
        {
            yield return ShowLabel(Draw_Color, "Egalit�", m_VictoryScreenDelay);
        }
